...
function upcomingDates(barber) {
  ...
  for (let i = 0; i < 21 && out.length < 14; i++) {
    ...
    if (workDays.includes(dayName)) out.push({ key: d.toISOString().slice(0, 10), label: i === 0 ? "Today" : dayName, sub: `${MONTHS[d.getMonth()]} ${d.getDate()}` });
  }
  return out;
}
...
export default function ShopBooking() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (session) {
        const { data: prof } = await supabase.from("customer_profiles").select("*")
          .eq("user_id", session.user.id).eq("shop_id", shopData.id).limit(1).single();
        if (prof) { setCustomer({ user: session.user, profile: prof }); setName(prof.name || ""); setPhone(prof.phone || ""); setEmail(prof.email || ""); }
      }
      setLoading(false);
    }
    if (slug) load();
  ...
  // availability via the safe busy_times function (no personal data)
  useEffect(() => {
  ...
  async function handleAuth() {
    ...
    try {
      if (authMode === "signup") {
        const { data: su, error: e1 } = await supabase.auth.signUp({ email: authEmail, password: authPass });
        if (e1) throw e1;
        let uid = su.user?.id;
        if (!su.session) { const { data: si, error: e2 } = await supabase.auth.signInWithPassword({ email: authEmail, password: authPass }); if (e2) throw e2; uid = si.user?.id; }
        await supabase.from("customer_profiles").insert({ user_id: uid, shop_id: shop.id, name: authName, phone: authPhone, email: authEmail });
      } else {
        const { error: e3 } = await supabase.auth.signInWithPassword({ email: authEmail, password: authPass });
        if (e3) throw e3;
      }
      const { data: { session } } = await supabase.auth.getSession();
      let { data: prof } = await supabase.from("customer_profiles").select("*").eq("user_id", session.user.id).eq("shop_id", shop.id).limit(1).single();
      if (!prof) {
        await supabase.from("customer_profiles").insert({ user_id: session.user.id, shop_id: shop.id, email: session.user.email });
        const r = await supabase.from("customer_profiles").select("*").eq("user_id", session.user.id).eq("shop_id", shop.id).limit(1).single();
        prof = r.data;
      }
  ...
  async function openHistory() {
    if (!customer) return;
    const { data } = await supabase.from("bookings").select("*").eq("customer_user_id", customer.user.id).order("created_at", { ascending: false });
    setHistory(data || []); setShowHistory(true);
  }

  const reset = () => { setStep("service"); setService(null); setBarber(null); setDate(null); setSlot(null); setDayBookings([]); if (!customer) { setName(""); setPhone(""); setEmail(""); } setOffers(false); };
    ...
    <div className="min-h-screen bg-stone-100 text-stone-900">
      <div className="mx-auto max-w-lg px-4 py-8">
        <div className="mb-3 flex items-center justify-end gap-3 text-sm">
          {customer ? (<>
            <button onClick={openHistory} className="font-medium text-emerald-700 hover:underline">My bookings</button>
            <button onClick={logoutCustomer} className="text-stone-500 hover:underline">Log out</button>
          </>) : (
        ...
        <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-stone-200">
          ...
          <div className="px-5 pb-5">
            <div className="-mt-8 mb-2 grid h-16 w-16 place-items-center rounded-2xl bg-emerald-600 text-2xl font-bold text-white shadow ring-4 ring-white">{shop.name[0]}</div>
        ...
        {showAuth && !customer && (
          <div className="mt-4 rounded-2xl bg-white p-4 ring-1 ring-emerald-300">
            <div className="mb-3 flex gap-2">
              <button onClick={() => setAuthMode("login")} className={`flex-1 rounded-lg py-2 text-sm font-medium ${authMode === "login" ? "bg-emerald-600 text-white" : "bg-stone-100 text-stone-600"}`}>Log in</button>
              <button onClick={() => setAuthMode("signup")} className={`flex-1 rounded-lg py-2 text-sm font-medium ${authMode === "signup" ? "bg-emerald-600 text-white" : "bg-stone-100 text-stone-600"}`}>Sign up</button>
            </div>
            <div className="space-y-2">
              {authMode === "signup" && <>
                <input value={authName} onChange={(e) => setAuthName(e.target.value)} placeholder="Your name" className={input} />
                <input value={authPhone} onChange={(e) => setAuthPhone(e.target.value)} placeholder="Mobile number" className={input} />
              </>}
              <input value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} placeholder="Email" type="email" className={input} />
              <input value={authPass} onChange={(e) => setAuthPass(e.target.value)} placeholder="Password (min 6)" type="password" className={input} />
            </div>
            {authErr && <p className="mt-2 text-sm text-red-600">{authErr}</p>}
            <div className="mt-3 flex gap-2">
              <button disabled={authBusy || !authEmail || !authPass} onClick={handleAuth} className="flex-1 rounded-xl bg-emerald-600 py-2.5 font-semibold text-white disabled:opacity-40">{authBusy ? "…" : authMode === "signup" ? "Create account" : "Log in"}</button>
              <button onClick={() => setShowAuth(false)} className="rounded-xl bg-stone-200 px-4 py-2.5 text-sm font-medium text-stone-700">Cancel</button>
            </div>
            <p className="mt-2 text-center text-xs text-stone-400">Optional — you can also just book as a guest below.</p>
          </div>
        )}

        {showHistory && customer && (
          <div className="mt-4 rounded-2xl bg-white p-4 ring-1 ring-stone-200">
            <div className="flex items-center justify-between"><h2 className="font-semibold">Your past bookings</h2><button onClick={() => setShowHistory(false)} className="text-sm text-stone-500">Close</button></div>
            {history.length === 0 ? <p className="mt-2 text-sm text-stone-500">No bookings yet.</p>
            : <div className="mt-2 space-y-2">{history.map((b) => (<div key={b.id} className="rounded-xl bg-stone-50 p-3 text-sm ring-1 ring-stone-200"><div className="font-medium">{b.service} with {b.barber}</div><div className="text-stone-500">{b.day} at {b.slot} · ${b.price}</div></div>))}</div>}
          </div>
        )}
        ...
        {step === "barber" && (
          <>
            ...
            : <div className="space-y-3">{staff.map((b) => (<div key={b.id} className="rounded-2xl bg-white p-4 ring-1 ring-stone-200"><div className="flex items-center gap-3"><div className={`grid h-11 w-11 place-items-center rounded-full ${b.color || "bg-emerald-700"} font-semibold text-white`}>{b.name[0]}</div><div><div className="font-semibold">{b.name}</div>{b.specialty && <span className="rounded-full bg-stone-100 px-2 py-0.5 text-xs text-stone-600">{b.specialty}</span>}</div></div><button onClick={() => { setBarber(b); setDate(null); setSlot(null); setStep("time"); }} className="mt-3 w-full rounded-xl bg-emerald-600 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700">Book with {b.name}</button></div>))}</div>}
          </>
        )}
        ...
        {step === "time" && (
          <>
            ...
            {dates.length === 0 ? <p className="mt-3 rounded-xl bg-white p-4 text-stone-500 ring-1 ring-stone-200">No working days set for this barber.</p>
            : <>
                <div className="mb-3 flex gap-2 overflow-x-auto pb-1">{dates.map((d) => (<button key={d.key} onClick={() => { setDate(d); setSlot(null); }} className={`min-w-[68px] shrink-0 rounded-xl px-2 py-2 text-center ring-1 transition ${date?.key === d.key ? "bg-emerald-600 text-white ring-emerald-600" : "bg-white text-stone-700 ring-stone-200"}`}><div className="text-sm font-semibold">{d.label}</div><div className={`text-xs ${date?.key === d.key ? "text-emerald-100" : "text-stone-400"}`}>{d.sub}</div></button>))}</div>
                {!date ? <p className="text-sm text-stone-400">Pick a day above.</p>
                : loadingSlots ? <p className="text-sm text-stone-400">Checking…</p>
                : availableSlots.length === 0 ? <p className="rounded-xl bg-white p-4 text-stone-500 ring-1 ring-stone-200">Fully booked — try another day.</p>
                : <div className="grid grid-cols-3 gap-2">{availableSlots.map((m) => (<button key={m} onClick={() => setSlot(m)} className={`rounded-xl py-2.5 text-sm font-medium ring-1 transition ${slot === m ? "bg-emerald-600 text-white ring-emerald-600" : "bg-white text-stone-800 ring-stone-200 hover:ring-emerald-400"}`}>{toLabel(m)}</button>))}</div>}
              </>}
            <button disabled={slot === null || !date} onClick={() => setStep("details")} className="mt-4 w-full rounded-xl bg-emerald-600 py-3 font-semibold text-white transition enabled:hover:bg-emerald-700 disabled:opacity-40">Continue</button>
          </>
        )}
        ...
        {step === "details" && (
          <>
            ...
            <button onClick={() => setOffers(!offers)} className="mt-2 flex w-full items-start gap-2 rounded-xl bg-white p-3 text-left text-sm ring-1 ring-stone-200"><span className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded border text-white ${offers ? "border-emerald-600 bg-emerald-600" : "border-stone-300"}`}>{offers ? "✓" : ""}</span><span className="text-stone-600">Email me offers &amp; news <span className="text-stone-400">— optional</span></span></button>
            <button disabled={!name || !phone || saving}
              onClick={async () => {
                ...
                if (clash) { setSaving(false); alert("Sorry, that time was just taken."); setStep("time"); setSlot(null); return; }
        ...
        {step === "done" && (
          <div className="mt-6 rounded-2xl bg-white p-6 text-center ring-1 ring-stone-200">
            ...
            {!customer && <p className="mt-3 text-sm text-stone-500">Want to rebook faster next time? <button onClick={() => { setStep("service"); setShowAuth(true); setAuthMode("signup"); }} className="font-medium text-emerald-700 underline">Create an account</button></p>}
            <button onClick={reset} className="mt-4 block w-full text-sm font-medium text-emerald-700 underline">Book another</button>
          </div>
        )}
      </div>
    </div>
