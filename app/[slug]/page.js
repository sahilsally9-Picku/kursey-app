  ...
  return (
    <div className="min-h-screen bg-stone-100 text-stone-900">
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        {step === "details" && (<><button onClick={() => setStep("time")} className="mt-6 text-sm font-medium text-stone-500">← Back</button><h2 className="mt-2 mb-3 text-lg font-semibold">Your details</h2><div className="rounded-2xl bg-white p-4 ring-1 ring-stone-200"><div className="flex justify-between text-sm"><span className="text-stone-400">Service</span><span className="font-medium">{service.name} · ${service.price}</span></div><div className="mt-1 flex justify-between text-sm"><span className="text-stone-400">Barber</span><span className="font-medium">{barber.name}</span></div><div className="mt-1 flex justify-between text-sm"><span className="text-stone-400">When</span><span className="font-medium">{date.label} {date.sub} at {toLabel(slot)}</span></div></div><div className="mt-3 space-y-2"><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" className={input} /><input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Mobile number" className={input} /><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email (confirmation & receipts)" className={input} /></div><button onClick={() => setOffers(!offers)} className="mt-2 flex w-full items-start gap-2 rounded-xl bg-white p-3 text-left text-sm ring-1 ring-stone-200"><span className={`mt-0.5 grid h-4 w-4 shrink-0 place-items-center rounded border text-white ${offers ? "border-emerald-600 bg-emerald-600" : "border-stone-300"}`}>{offers ? "✓" : ""}</span><span className="text-stone-600">Email me offers &amp; news <span className="text-stone-400">— optional</span></span></button><button disabled={!name || !phone || saving} onClick={async () => { setSaving(true); const dur = service.mins || 30; const { data: fresh } = await supabase.rpc("busy_times", { p_shop_id: shop.id, p_barber: barber.name, p_date: date.key }); const clash = (fresh || []).some((b) => b.start_min != null && overlaps(slot, slot + dur, b.start_min, b.start_min + (b.duration_min || 30))); if (clash) { setSaving(false); alert("Sorry, that time was just taken."); setStep("time"); setSlot(null); return; } const { error } = await supabase.from("bookings").insert({ shop_id: shop.id, service: service.name, price: service.price, barber: barber.name, day: `${date.label} ${date.sub}`, slot: toLabel(slot), booking_date: date.key, start_min: slot, duration_min: dur, customer_name: name, phone: phone, email: email, wants_offers: offers, customer_user_id: customer ? customer.user.id : null }); setSaving(false); if (error) { alert("Something went wrong: " + error.message); } else { setStep("done"); } }} className="mt-3 w-full rounded-xl bg-emerald-600 py-3 font-semibold text-white transition enabled:hover:bg-emerald-700 disabled:opacity-40">{saving ? "Booking..." : "Confirm booking"}</button></>)}

        {step === "done" && (<div className="mt-6 rounded-2xl bg-white p-6 text-center ring-1 ring-stone-200"><div className="mx-auto mb-3 grid h-14 w-14 place-items-center rounded-full bg-emerald-100 text-2xl text-emerald-700">✓</div><h2 className="text-xl font-bold">You&apos;re booked, {name}!</h2><p className="mt-1 text-stone-600"><span className="font-semibold">{service.name}</span> with <span className="font-semibold">{barber.name}</span></p><p className="mt-1 text-stone-600">{date.label} {date.sub} at {toLabel(slot)}</p>{!customer && <p className="mt-3 text-sm text-stone-500">Want to rebook faster next time? <button onClick={() => { setStep("service"); setShowAuth(true); setAuthMode("signup"); }} className="font-medium text-emerald-700 underline">Create an account</button></p>}<button onClick={reset} className="mt-4 block w-full text-sm font-medium text-emerald-700 underline">Book another</button></div>)}
      </div>
    </div>
  );
}