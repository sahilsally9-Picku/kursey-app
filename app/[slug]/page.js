...
export default function ShopBooking() {
  ...
  const shownSlots = [...new Set([...availableSlots, ...openClassTimes])].sort((a, b) => a - b);
  ...
  async function saveBooking(depositPaid, paymentIntentId) {
    ...
    if (isClass) {
      ...
      if (takenNow >= classCap) { setSaving(false); alert("Sorry, that class just filled up."); setStep("time"); setSlot(null); setClientSecret(null); return false; }
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="mx-auto max-w-lg px-4 py-8">
              ...
              return (
                <button key={s.id} onClick={() => toggleService(s)} className={`flex w-full items-start justify-between rounded-xl bg-white p-4 text-left transition ${on ? "ring-2 ring-[#13294b]" : "ring-1 ring-slate-200 hover:ring-[#13294b]"}`}>
                  <div className="flex items-start gap-3 pr-3">
                    ...
                    <div><div className="font-medium">{s.name}</div><div className="text-sm text-slate-500">{s.mins} min{s.capacity > 1 ? ` · group class of ${s.capacity}` : ""}</div>{s.description && <div className="mt-1 text-sm text-slate-400">{s.description}</div>}</div>
                  </div>
        ...
        {step === "time" && (<><button onClick={() => { setStep("barber"); setDate(null); setSlot(null); }} className="mt-6 text-sm font-medium text-slate-500">← Back</button><h2 className="mt-2 mb-1 text-lg font-semibold">Pick a day &amp; time with {barber.name}</h2><p className="mb-3 text-sm text-slate-500">{serviceNames} · {totalMins} min{gapMin > 0 ? ` (includes ${gapMin} min processing)` : ""}{isClass ? ` · group class of ${classCap}` : ""}</p>{dates.length === 0 ? <p className="mt-3 rounded-xl bg-white p-4 text-slate-500 ring-1 ring-slate-200">No working days set for this {t.staff}.</p> : <><div className="mb-3 flex gap-2 overflow-x-auto pb-1">{dates.map((d) => (<button key={d.key} onClick={() => { setDate(d); setSlot(null); }} className={`min-w-[68px] shrink-0 rounded-xl px-2 py-2 text-center ring-1 transition ${date?.key === d.key ? "bg-[#13294b] text-white ring-[#13294b]" : "bg-white text-slate-700 ring-slate-200"}`}><div className="text-sm font-semibold">{d.label}</div><div className={`text-xs ${date?.key === d.key ? "text-white/70" : "text-slate-400"}`}>{d.sub}</div></button>))}</div>{!date ? <p className="text-sm text-slate-400">Pick a day above.</p> : loadingSlots ? <p className="text-sm text-slate-400">Checking…</p> : shownSlots.length === 0 ? <p className="rounded-xl bg-white p-4 text-slate-500 ring-1 ring-slate-200">Fully booked — try another day.</p> : <div className="grid grid-cols-3 gap-2">{shownSlots.map((m) => { const left = isClass ? classCap - (spotsTaken[m] || 0) : null; return (<button key={m} onClick={() => setSlot(m)} className={`rounded-xl py-2.5 text-sm font-medium ring-1 transition ${slot === m ? "bg-[#13294b] text-white ring-[#13294b]" : "bg-white text-slate-800 ring-slate-200 hover:ring-[#13294b]"}`}><div>{toLabel(m)}</div>{isClass && <div className={`text-[10px] ${slot === m ? "text-white/70" : "text-slate-500"}`}>{left} left</div>}</button>); })}</div>}</>}<button disabled={slot === null || !date} onClick={() => setStep("details")} className="mt-4 w-full rounded-xl bg-[#13294b] py-3 font-semibold text-white transition enabled:hover:bg-[#1d3a63] disabled:opacity-40">Continue</button></>)}
