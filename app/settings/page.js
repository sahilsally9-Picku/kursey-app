...
export default function Settings() {
  ...
  const [sName, setSName] = useState(""); const [sPrice, setSPrice] = useState(""); const [sMins, setSMins] = useState("");
  const [editServiceId, setEditServiceId] = useState(null);

  const [staff, setStaff] = useState([]);
  const [stName, setStName] = useState(""); const [stSpecialty, setStSpecialty] = useState("");
  const [stBio, setStBio] = useState(""); const [stPhoto, setStPhoto] = useState("");
  ...
  // generic image upload → returns public url
  ...
  async function uploadLogo(file) {
    if (!file) return; setUploadingLogo(true);
    try { const url = await uploadImage(file, "logos"); await supabase.from("shops").update({ logo_url: url }).eq("id", shop.id); setShop({ ...shop, logo_url: url }); }
  ...
  async function uploadStaffPhoto(file) {
    if (!file) return; setUploadingPhoto(true);
    try { const url = await uploadImage(file, "barbers"); setStPhoto(url); }
  ...
  function resetStaffForm() { setStName(""); setStSpecialty(""); setStBio(""); setStPhoto(""); setStColor("bg-emerald-700"); setStDays(["Mon", "Tue", "Wed", "Thu", "Fri"]); setStStart("09:00"); setStEnd("17:00"); setStBreaks([]); setBrStart("13:00"); setBrEnd("14:00"); setEditStaffId(null); }
  function startEditStaff(st) { setEditStaffId(st.id); setStName(st.name); setStSpecialty(st.specialty || ""); setStBio(st.bio || ""); setStPhoto(st.photo_url || ""); setStColor(st.color || "bg-emerald-700"); setStDays(st.work_days || []); setStStart(st.start_time || "09:00"); setStEnd(st.end_time || "17:00"); setStBreaks(st.breaks || []); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  async function saveStaff() {
    ...
    const payload = { name: stName, specialty: stSpecialty, bio: stBio, photo_url: stPhoto || null, color: stColor, work_days: stDays, start_time: stStart, end_time: stEnd, breaks: stBreaks, shop_id: shop.id };
  ...
  return (
    <div className="min-h-screen bg-stone-100 text-stone-900">
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        {/* LOGO */}
        <h2 className="mt-6 mb-2 text-lg font-semibold">Shop logo</h2>
        <div className="flex items-center gap-4 rounded-2xl bg-white p-4 ring-1 ring-stone-200">
          <div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-xl bg-emerald-600 text-2xl font-bold text-white">{shop.logo_url ? <img src={shop.logo_url} alt="logo" className="h-full w-full object-cover" /> : shop.name[0]}</div>
          <div className="flex-1">
            <label className="inline-block cursor-pointer rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700">{uploadingLogo ? "Uploading…" : shop.logo_url ? "Replace logo" : "Upload logo"}<input type="file" accept="image/*" className="hidden" onChange={(e) => uploadLogo(e.target.files[0])} disabled={uploadingLogo} /></label>
            {shop.logo_url && <button onClick={removeLogo} className="ml-3 text-sm text-red-600 hover:underline">Remove</button>}
            <p className="mt-1 text-xs text-stone-400">Shows on your booking page. Square images look best.</p>
          </div>
        </div>
        ...
        <div className={`rounded-2xl bg-white p-4 ring-1 ${editServiceId ? "ring-emerald-400" : "ring-stone-200"}`}>
          ...
          <div className="space-y-2"><input value={sName} onChange={(e) => setSName(e.target.value)} placeholder="Service name (e.g. Skin Fade)" className={input} /><div className="flex gap-2"><input value={sPrice} onChange={(e) => setSPrice(e.target.value)} placeholder="Price $" type="number" className={input} /><input value={sMins} onChange={(e) => setSMins(e.target.value)} placeholder="Minutes" type="number" className={input} /></div></div>
        ...
        {services.length > 0 && <div className="mt-2 space-y-2">{services.map((s) => (<div key={s.id} className="flex items-center justify-between rounded-xl bg-white p-4 ring-1 ring-stone-200"><div><div className="font-medium">{s.name}</div><div className="text-sm text-stone-500">{s.mins} min · ${s.price}</div></div><div className="flex items-center gap-3"><button onClick={() => startEditService(s)} className="text-sm text-emerald-700 hover:underline">Edit</button><button onClick={() => deleteService(s.id)} className="text-sm text-red-600 hover:underline">Remove</button></div></div>))}</div>}
        ...
        <div className={`rounded-2xl bg-white p-4 ring-1 ${editStaffId ? "ring-emerald-400" : "ring-stone-200"}`}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-full bg-stone-200 text-xs text-stone-400">{stPhoto ? <img src={stPhoto} alt="" className="h-full w-full object-cover" /> : "Photo"}</div>
              <label className="cursor-pointer rounded-xl bg-stone-800 px-3 py-2 text-sm font-semibold text-white">{uploadingPhoto ? "Uploading…" : stPhoto ? "Change photo" : "Upload photo"}<input type="file" accept="image/*" className="hidden" onChange={(e) => uploadStaffPhoto(e.target.files[0])} disabled={uploadingPhoto} /></label>
              {stPhoto && <button onClick={() => setStPhoto("")} className="text-sm text-red-600 hover:underline">Remove</button>}
            </div>
            ...
            <textarea value={stBio} onChange={(e) => setStBio(e.target.value)} placeholder="About this barber — experience, style, anything clients should know" rows={3} className={`${input} resize-none`} />
