...
function SettingsInner() {
  ...
  const [sGapAfter, setSGapAfter] = useState(""); const [sGap, setSGap] = useState("");
  ...
  function resetServiceForm() { setSName(""); setSPrice(""); setSMins(""); setSDesc(""); setSGapAfter(""); setSGap(""); setEditServiceId(null); }
  function startEditService(s) { setEditServiceId(s.id); setSName(s.name); setSPrice(String(s.price)); setSMins(String(s.mins || "")); setSDesc(s.description || ""); setSGapAfter(s.gap_after_min ? String(s.gap_after_min) : ""); setSGap(s.gap_min ? String(s.gap_min) : ""); window.scrollTo({ top: 0, behavior: "smooth" }); }
  async function saveService() {
    if (!sName || !sPrice) return;
    const totalM = sMins ? parseInt(sMins) : 30;
    const gapA = parseInt(sGapAfter) || 0;
    const gapG = parseInt(sGap) || 0;
    if ((gapA > 0) !== (gapG > 0)) { alert("For processing time, fill in both the hands-on minutes and the gap length — or leave both blank."); return; }
    if (gapA + gapG >= totalM) { alert(`Hands-on (${gapA}) + gap (${gapG}) must be less than the total ${totalM} minutes, so there's finishing time left.`); return; }
    const payload = { name: sName, price: parseInt(sPrice), mins: totalM, description: sDesc, shop_id: shop.id, gap_after_min: gapA, gap_min: gapG };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        <div className={`p-4 ${editServiceId ? "rounded-2xl bg-white ring-1 ring-[#13294b]/30 shadow-sm" : card}`}>
          ...
          <div className="space-y-2"><input value={sName} onChange={(e) => setSName(e.target.value)} placeholder="Service name" className={input} /><div className="flex gap-2"><input value={sPrice} onChange={(e) => setSPrice(e.target.value)} placeholder="Price $" type="number" className={input} /><input value={sMins} onChange={(e) => setSMins(e.target.value)} placeholder="Minutes" type="number" className={input} /></div><textarea value={sDesc} onChange={(e) => setSDesc(e.target.value)} placeholder="Description (optional)" rows={2} className={`${input} resize-none`} /><div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><div className="text-xs font-medium text-slate-700">Processing time <span className="font-normal text-slate-500">— optional</span></div><p className="mt-0.5 text-xs text-slate-500">For colour, perms and similar: how long you're hands-on first, then how long the client processes while you're free. Leave blank for normal services.</p><div className="mt-2 flex gap-2"><input value={sGapAfter} onChange={(e) => setSGapAfter(e.target.value)} placeholder="Hands-on min" type="number" className={input} /><input value={sGap} onChange={(e) => setSGap(e.target.value)} placeholder="Gap min" type="number" className={input} /></div></div></div>
        ...
        {services.length > 0 && <div className="mt-2 space-y-2">{services.map((s) => (<div key={s.id} className={`flex items-start justify-between p-4 ${card}`}><div className="pr-3"><div className="font-medium">{s.name}</div><div className="text-sm text-slate-600">{s.mins} min · ${s.price}</div>{s.gap_min > 0 && s.gap_after_min > 0 && <div className="text-xs font-medium text-[#13294b]">{s.gap_after_min} min hands-on · {s.gap_min} min processing · {s.mins - s.gap_after_min - s.gap_min} min finish</div>}{s.description && <div className="mt-0.5 font-display text-xs italic text-slate-500">{s.description}</div>}</div><div className="flex shrink-0 items-center gap-3"><button onClick={() => startEditService(s)} className="text-sm text-[#13294b] hover:underline">Edit</button><button onClick={() => deleteService(s.id)} className="text-sm text-red-600 hover:underline">Remove</button></div></div>))}</div>}
