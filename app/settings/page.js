...
function SettingsInner() {
  ...
  function resetServiceForm() { setSName(""); setSPrice(""); setSMins(""); setSDesc(""); setSGapAfter(""); setSGap(""); setSCap(""); setEditServiceId(null); }
  function startEditService(s) { setEditServiceId(s.id); setSName(s.name); setSPrice(String(s.price)); setSMins(String(s.mins || "")); setSDesc(s.description || ""); setSGapAfter(s.gap_after_min ? String(s.gap_after_min) : ""); setSGap(s.gap_min ? String(s.gap_min) : ""); setSCap(s.capacity && s.capacity > 1 ? String(s.capacity) : ""); window.scrollTo({ top: 0, behavior: "smooth" }); }
  async function saveService() {
    ...
    const capN = Math.max(1, parseInt(sCap) || 1);
    const payload = { name: sName, price: parseInt(sPrice), mins: totalM, description: sDesc, shop_id: shop.id, gap_after_min: gapA, gap_min: gapG, capacity: capN };
